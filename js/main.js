// INITIALISATION DE L'INSTANCE DE VUE JS
new Vue({
  el: "#app",
  data: {
    tab: null,
    ...
    items: [
      "Formation Java",
      "Entreprenariat DNS",
      "Lancement Women and Girls Startup Challenge",
    ...
    slides: [
      //   {
      //     src:
      //       "img/csm_concours_nueva_centre_jacques_cartier_cjc-01_73884909b9.png"
      //   },
      {
        src: "img/slide5.jpg"
      }
  ...
  methods: {
    //LOADER
    pageLoader() {
      // Déclanchons le loader au chargement de la page
      jQuery(".loader").fadeOut(800);
    },
    galeriePhoto() {
      // Lancement de la galerie photo
      /*Flat three columns*/
      for (let item of this.items) {
        $("#flat-gallery-" + this.items.indexOf(item)).cubeportfolio({
          layoutMode: "grid",
    ...
    contactForm() {
      $("#submit_btn").click(function() {
        // Récuperons les valeures des champs
        let user_name =
        ...
        //Verifions la validité des champs
        ...
        if (proceed) {
          ...
          $.post(
            ...
            post_data,
            function(response) {
              //Récupérons la reponse du serveur et affichons le message
              if (response.type == "error") {
                ...
                //effaçons les anciennes valeures
          ...
          output =
            '<div class="alert-danger" style="padding:10px; margin-bottom:25px;">Veuillez renseigner tout les champs SVP.</div>';
          $("#result")
            .hide()
            .html(output)
            .slideDown();
    ...
    // Reste des scripts Jquery du template
    jQueryAction() {
      jQuery(function($) {
        "use strict";
        var $window = $(window);
  ...
  mounted() {
    ...
    // Lancement du script de la galerie photo
    this.galeriePhoto();

    // Lancement de tout les scripts Jquery du template
    this.jQueryAction();
  },
  watch: {
    loader() {
